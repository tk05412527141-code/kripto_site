import"./background-3d-Dh-7dz6s.js";document.addEventListener("DOMContentLoaded",()=>{const i=document.getElementById("assets-list"),f=t=>{const e=parseFloat(t);return e<.1?e.toLocaleString("tr-TR",{minimumFractionDigits:4,maximumFractionDigits:5}):e<1?e.toLocaleString("tr-TR",{minimumFractionDigits:3,maximumFractionDigits:4}):e.toLocaleString("tr-TR",{minimumFractionDigits:2,maximumFractionDigits:2})},u=t=>t?'<svg class="w-full h-full stroke-secondary drop-shadow-[0_0_4px_#69f6b8]" viewBox="0 0 100 40"><path d="M0,30 Q25,10 50,25 T100,5" fill="none" stroke-width="2.5"></path></svg>':'<svg class="w-full h-full stroke-error drop-shadow-[0_0_4px_#ff6e84]" viewBox="0 0 100 40"><path d="M0,10 Q25,35 50,20 T100,35" fill="none" stroke-width="2.5"></path></svg>';async function l(){const t="https://api.coingecko.com/api/v3/coins/markets?vs_currency=try&order=market_cap_desc&per_page=100&page=1&sparkline=false&locale=tr";try{const e=await fetch(t);if(!e.ok)throw new Error(`HTTP error! status: ${e.status}`);const s=await e.json();i&&(i.innerHTML=s.map((r,a)=>{const n=(r.price_change_percentage_24h||0)>=0,o=r.symbol.toUpperCase(),g=r.image,h="./logo.png",m=u(n),v=n?"text-secondary":"text-error",x=n?"+":"";let p="";a===0&&(p=`
                  <span class="bg-[#bd9dff]/10 text-[#bd9dff] text-[9px] px-2 py-0.5 rounded-full uppercase tracking-widest font-bold flex items-center gap-1 border border-[#bd9dff]/20 shadow-[0_0_8px_rgba(189,157,255,0.3)]">
                    ...
                    Günün Yıldızı
                  </span>`);const w=o+"TRY";return`
                <div class="asset-row-wrapper group flex flex-col bg-surface-container-low rounded-xl border border-transparent hover:border-primary/20 transition-all overflow-hidden mb-3" data-symbol="${o}">
                  <div class="flex items-center justify-between p-4 cursor-pointer" onclick="toggleTradingViewChart(this, '${w}')">
                    <div class="flex items-center gap-4 flex-1">
                      <div class="w-12 h-12 rounded-full bg-surface-container-highest flex items-center justify-center p-1.5 shadow-inner border border-outline-variant/10">
                        <img src="${g}" onerror="this.src='${h}'" alt="${o}" class="w-full h-full object-contain" />
                      </div>
                      <div>
                        <div class="flex items-center gap-2 mb-0.5">
                           <h4 class="font-headline font-bold text-on-surface leading-none">${o}</h4>
                           ${p}
                        </div>
              ...
              `}).join(""))}catch(e){console.error("CoinGecko data fetch failed:",e)}}window.toggleTradingViewChart=function(t,e,s=!1){const a=t.closest(".asset-row-wrapper").querySelector(".chart-container"),n=a.style.height!==""&&a.style.height!=="0px";document.querySelectorAll(".chart-container").forEach(o=>{o.style.height="0px",o.innerHTML=""}),!(n&&!s)&&(a.style.height="500px",a.innerHTML=`<div class="tradingview-widget-container h-full w-full border-t border-surface-container-high relative"><div id="tv_${e}" class="h-full w-full"></div></div>`,new window.TradingView.widget({autosize:!0,symbol:"BINANCE:"+e,interval:"D",timezone:"Etc/UTC",theme:"dark",style:"1",locale:"tr",enable_publishing:!1,backgroundColor:"#0c1222",gridColor:"rgba(189, 157, 255, 0.05)",hide_top_toolbar:!1,hide_legend:!1,save_image:!1,container_id:"tv_"+e}))};const c=document.createElement("script");c.src="https://s3.tradingview.com/tv.js",document.head.appendChild(c);const d=document.getElementById("al-search");d&&d.addEventListener("input",t=>{const e=t.target.value.toLowerCase().trim();i.querySelectorAll(".asset-row-wrapper").forEach(r=>{const a=r.getAttribute("data-symbol").toLowerCase();r.style.display=a.includes(e)?"flex":"none"})}),l(),setInterval(l,6e4)});
